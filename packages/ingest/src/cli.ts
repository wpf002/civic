...
program
  ...
  .action(async (o) => {
    ...
    // State offices a list names, by race key. Resolved by title and seat label.
    let stateOffices = new Map<string, OfficeSpec>();
    ...
    } else if (o.adapter === "fl-dos") {
      const run = await fetchFlRoster(new Date());
      rosters = run.rosters;
      stateOffices = run.offices;
      // The Division's own status: Qualified after the primary is the ballot.
      basis = "CERTIFIED";
      console.log(`basis: CERTIFIED — ${run.candidateCount} qualified candidates in ${run.rosters.length} contests`);
      resolve = async (raceKey: string) => {
        const { raceId, reason } = await resolveFederalSeat(o.election, raceKey);
        if (!raceId) console.log(`  ! ${raceKey}: ${reason}`);
        return raceId;
    ...
    const federal = resolve;
    const out = await persistRun(
      { adapter: o.adapter, electionSlug: o.election, basis, dryRun: !!o.dryRun },
      rosters,
      async (raceKey: string) => {
        const spec = stateOffices.get(raceKey);
        if (!spec) return federal(raceKey);
...
program
  ...
  .action(async (o) => {
    const now = new Date();
    const lists: Record<string, () => Promise<{ offices: Map<string, OfficeSpec> }>> = {
      "tx-sos": () => fetchCertifiedRoster(NOVEMBER_2026, now),
      "nc-sbe": () => fetchNcRoster(NC_NOVEMBER_2026, now),
      "mn-sos": () => fetchMnRoster(MN_NOVEMBER_2026, now),
    ...
    const state = o.adapter.slice(0, 2).toUpperCase();
    const { offices } = await load();
    const upper = [...offices.values()].filter((s) => s.chamber === "upper").length;
    const lower = [...offices.values()].filter((s) => s.chamber === "lower").length;
    console.log(`${state}: ${offices.size} state offices on the list: ${offices.size - upper - lower} statewide, ${upper} senate, ${lower} house`);
    if (o.dryRun) {
      for (const [k, s] of offices) console.log(`  ${k}: ${s.title} / ${s.seatLabel}`);
    } else {
      const created = await ensureStateOffices(o.election, state, offices);
      console.log(`created ${created.length} races`);
      for (const c of created.filter((c) => c.startsWith("statewide"))) console.log(`  + ${c}`);
    }
    await prisma.$disconnect();
  });
