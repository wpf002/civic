...
5. **Phase 0 fidelity test — run, with a caveat.** Results in `docs/PHASE0_RESULTS.md`,
   regenerated by `pnpm extract fidelity`. It passes the roadmap's 80% stance-agreement
   kill criterion at 88.9%, but the labels it scores against were written by models, not
   by a person, so that number measures convergence rather than correctness. The 15
   contested rows listed in the results are where a human reader would actually change
   the outcome.
